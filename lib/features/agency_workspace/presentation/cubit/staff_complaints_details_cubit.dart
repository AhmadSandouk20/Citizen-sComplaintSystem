...
class StaffComplaintDetailsCubit extends Cubit<StaffComplaintDetailsState> {
    ...
    try {
      ...
      emit(
        state.copyWith(
          status: StaffComplaintDetailsStatus.success,
          complaint: complaint,
          successMessage: 'تم قفل الشكوى بنجاح',
        ),
      );
    } on AppException catch (e) {
      emit(
    ...
    try {
      ...
      emit(
        state.copyWith(
          status: StaffComplaintDetailsStatus.success,
          complaint: complaint,
          successMessage: 'تم فك قفل الشكوى',
        ),
      );
    } on AppException catch (e) {
      emit(
    ...
    try {
      ...
      emit(
        state.copyWith(
          status: StaffComplaintDetailsStatus.success,
          complaint: complaint,
          successMessage: 'تم تحديث الشكوى',
        ),
      );
    } on AppException catch (e) {
      emit(
  ...
  Future<bool> requestMoreInfo({
    ...
    if (message.trim().isEmpty) {
      emit(
        state.copyWith(
          errorMessage: 'يرجى كتابة الرسالة أولاً',
          clearSuccess: true,
        ),
      );
      return false;
    ...
    try {
      ...
      emit(
        state.copyWith(
          status: StaffComplaintDetailsStatus.success,
          successMessage: 'تم إرسال طلب المعلومات إلى المواطن بنجاح',
          clearError: true,
        ),
      );
      ...
      emit(
        state.copyWith(
          status: StaffComplaintDetailsStatus.success,
          errorMessage: 'تعذر إرسال طلب المعلومات الإضافية',
          clearSuccess: true,
        ),
      );
