...
class NotificationsCubit extends Cubit<NotificationsState> {
  ...
  Future<void> markAllAsRead() async {
    ...
    try {
      final failed = await _repository.markAllAsRead(unreadIds);
      await load(refresh: true);

      // Partial success must not look like full success — the old version
      // broke out of the loop on the first failure and said nothing.
      if (failed > 0) {
        emit(
          NotificationsError(
            'Could not mark $failed of ${unreadIds.length} notifications.',
            unreadCount: state.unreadCount,
          ),
        );
      }
    } catch (error) {
      emit(NotificationsError(_message(error), unreadCount: current.unreadCount));
    }
  }
